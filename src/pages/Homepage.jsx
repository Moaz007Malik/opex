...
function HeroForm() {
  ...
  if (submitted) {
    return (
      <div className="bg-card-bg border border-slate-700 rounded-xl p-6 text-center">
        <p className="text-text-primary font-medium">Thank you. You're on the list.</p>
        <p className="text-text-secondary text-sm mt-1">We'll be in touch ahead of launch.</p>
      </div>
    );
  }

  return (
    <form onSubmit={handleSubmit} className="bg-card-bg border border-slate-700 rounded-xl p-6 space-y-4 max-w-md mx-auto">
    ...
    <>
      ...
      <section className="py-12 bg-card-bg border-y border-border">
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div>
              <p className="text-2xl lg:text-3xl font-bold text-text-primary">15+</p>
              <p className="text-text-secondary text-sm mt-1">KPI Categories — structured across a single dashboard framework</p>
            </div>
            <div>
              <p className="text-2xl lg:text-3xl font-bold text-text-primary">100+</p>
              <p className="text-text-secondary text-sm mt-1">individual KPI metrics — built into the Exec App at launch</p>
            </div>
            <div>
              <p className="text-2xl lg:text-3xl font-bold text-text-primary">1</p>
              <p className="text-text-secondary text-sm mt-1">platform — designed to replace fragmented spreadsheets, reports, and disconnected systems</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <Section className="bg-secondary/50">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {SECTORS.map((s, i) => (
            <Card key={i}>
              <h3 className="font-semibold text-lg text-text-primary mb-2">{s.name}</h3>
          ...
          <table className="w-full border-collapse min-w-[600px]">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-4 px-4 text-text-primary font-semibold">The visibility challenge</th>
            ...
            <tbody>
              {PROBLEM_SOLUTION.map((row, i) => (
                <tr key={i} className="border-b border-border">
                  <td className="py-4 px-4 text-text-secondary text-sm">{row.challenge}</td>
                  <td className="py-4 px-4 text-text-secondary text-sm">{row.solution}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {FEATURE_CARDS.map((card, i) => (
            <Card key={i}>
              ...
              <h3 className="font-semibold text-lg text-text-primary mb-2">{card.title}</h3>
              <p className="text-text-secondary text-sm">{card.copy}</p>
            </Card>
          ))}
        </div>
      </Section>
      ...
      <Section className="bg-secondary/50">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {LEADERSHIP_CHALLENGES.map((c, i) => (
            <Card key={i}>
              <p className="text-text-secondary text-sm mb-3">&ldquo;{c.statement}&rdquo;</p>
      ...
      <Section>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mb-6">
          {INTEGRATION_BRANDS.map((brand, i) => (
            <div key={i} className="h-14 rounded-lg bg-slate-800 border border-slate-600 flex items-center justify-center">
              <span className="text-xs text-slate-400 font-medium grayscale opacity-80">{brand}</span>
            </div>
          ))}
        </div>
